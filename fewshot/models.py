...
class RoboCentricWorldModel(nn.Module):
    ...
    def _train_head_only(self, data):
        data = self.preprocess(data)
        self.heads['reward'].requires_grad_(requires_grad=True)
        self.heads['cont'].requires_grad_(requires_grad=True)
        ...
        with torch.cuda.amp.autocast(self._use_amp):
            ...
            preds = {}
            for name, head in self.heads.items():
                if 'reward' in name or 'cont' in name:
                    grad_head = name in self._config.grad_heads
                    feat = self.concat_merge_feat(robot_post, env_post)
                    feat = feat if grad_head else feat.detach()
                    pred = head(feat)
    ...
    def _train_env_branch(self, data):
        data = self.preprocess(data)
        self.encoder._cnn2.requires_grad_(requires_grad=True)
        self.dynamics._env_inp_layers.requires_grad_(requires_grad=True)
        self.dynamics._env_cell.requires_grad_(requires_grad=True)
        self.dynamics._env_img_out_layers.requires_grad_(requires_grad=True)
        self.dynamics._env_obs_out_layers.requires_grad_(requires_grad=True)
        self.dynamics._env_ims_stat_layer.requires_grad_(requires_grad=True)
        self.dynamics._env_obs_stat_layer.requires_grad_(requires_grad=True)
        self.dynamics.env_W.requires_grad_(requires_grad=True)
        self.dynamics.robot_env_atten.requires_grad_(requires_grad=True)
        self.heads['decoder']._env_decoder.requires_grad_(requires_grad=True)
        ...
        with torch.cuda.amp.autocast(self._use_amp):
            ...
            robot_feat = self.dynamics.get_feat(robot_post)
            env_feat = self.dynamics.get_feat(env_post)
            pred, robot_recon, env_recon, robot_mask, env_mask = self.heads['decoder'](robot_feat, env_feat)
            if type(pred) is dict:
                preds.update(pred)
            else:
                preds['decoder'] = pred
            ...
            model_loss = sum(losses.values()) + env_kl_loss
        metrics = self._model_opt(model_loss, self.parameters())
...
class ImagBehavior(nn.Module):
    ...
    def _train_feg(
        ...
        self._update_slow_target()
        metrics = {}
        exp_data = self._world_model.preprocess(exp_data)

        with tools.RequiresGrad(self.actor):
            with torch.cuda.amp.autocast(self._use_amp):
                # Behavioral cloning
                bc_loss = self._compute_bc_loss(exp_data)
                ...
                actor_loss = bc_loss * self._bc_scale + actor_loss * self._rl_scale

                if self._config.use_decay_bc_scale:
                    self._bc_scale = max(self._config.bc_scale_min,
                                         self._bc_scale - (
                                                 self._config.bc_scale - self._config.bc_scale_min) / self._config.decay_step)

                    self._rl_scale = min(self._config.rl_scale_max,
                                         self._rl_scale + (
                                                 self._config.rl_scale_max - self._config.rl_scale) / self._config.decay_step)
                    metrics['bc_scale'] = self._bc_scale
                    metrics['rl_scale'] = self._rl_scale
    ...
    def _compute_bc_loss(self, actions, robot_start, env_start):
        actions = torch.tensor(actions).to('cuda')
        actions = actions[:, 1:]
        feat = self._world_model.concat_merge_feat(robot_start, env_start)
        feat = feat[:, :-1]
        pred = self.actor(feat)
        like = pred.log_prob(actions)
